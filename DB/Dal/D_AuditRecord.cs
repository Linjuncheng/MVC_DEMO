            ...
            using (conn = CreatConn())
            {
                ...
                try
                {
                    ...
                    foreach (int i in ID)
                    {
                        ...
                        while (sdr.Read())
                        {
                            M_AuditRecord TmpObj = new M_AuditRecord();
                            TmpObj.ID = (int)sdr["ID"];
                            TmpObj.Employee_ID = (int)sdr["Employee_ID"];
                            TmpObj.FormType = (string)sdr["FormType"];
                            TmpObj.HistoricalRecord = (string)sdr["HistoricalRecord"];
                            Obj.Add(TmpObj);
                            tmpOut++;
                        }
                        sdr.Close();
