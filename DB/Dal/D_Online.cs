            ...
            using (conn = CreatConn())
            {
                ...
                try
                {
                    ...
                    foreach (int i in ID)
                    {
                        ...
                        while (sdr.Read())
                        {
                            M_Online TmpObj = new M_Online();
                            TmpObj.ID = (int)sdr["ID"];
                            TmpObj.Admin_BL = (int)sdr["Admin_BL"];
                            if (sdr["Admin_ID"] == DBNull.Value) { TmpObj.Admin_ID = null; } else { TmpObj.Admin_ID = (int)sdr["Admin_ID"]; }
                            TmpObj.LastLogin = (DateTime)sdr["LastLogin"];
                            TmpObj.PastTime = (long)sdr["PastTime"];
                            if (sdr["Token"] == DBNull.Value) { TmpObj.Token = null; } else { TmpObj.Token = (string)sdr["Token"]; }
            ...
            using (conn = CreatConn())
            {
                ...
                try
                {
                    ...
                    while (sdr.Read())
                    {
                        ...
                        if (sdr["Admin_ID"] == DBNull.Value) { Obj.Admin_ID = null; } else { Obj.Admin_ID = (int)sdr["Admin_ID"]; }
                        Obj.LastLogin = (DateTime)sdr["LastLogin"];
                        Obj.PastTime = (long)sdr["PastTime"];
                        if (sdr["Token"] == DBNull.Value) { Obj.Token = null; } else { Obj.Token = (string)sdr["Token"]; }
