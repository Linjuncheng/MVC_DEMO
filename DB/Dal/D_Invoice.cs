            ...
            using (conn = CreatConn())
            {
                ...
                try
                {
                    ...
                    foreach (int i in ID)
                    {
                        ...
                        while (sdr.Read())
                        {
                            M_Invoice TmpObj = new M_Invoice();
                            TmpObj.ID = (int)sdr["ID"];
                            TmpObj.BankNumber = (string)sdr["BankNumber"];
                            if (sdr["InvoiceCompanyName"] == DBNull.Value) { TmpObj.InvoiceCompanyName = string.Empty; } else { TmpObj.InvoiceCompanyName = (string)sdr["InvoiceCompanyName"]; }
                            TmpObj.InvoiceNumber = (string)sdr["InvoiceNumber"];
                            TmpObj.InvoicePrice = (int)sdr["InvoicePrice"];
                            TmpObj.InvoiceTime = (DateTime)sdr["InvoiceTime"];
                            TmpObj.Purchase_ID = (int)sdr["Purchase_ID"];
                            TmpObj.Remark = (string)sdr["Remark"];
                            Obj.Add(TmpObj);
                            tmpOut++;
                        }
                        sdr.Close();
            ...
            using (conn = CreatConn())
            {
                ...
                try
                {
                    ...
                    while (sdr.Read())
                    {
                        ...
                        if (sdr["InvoiceCompanyName"] == DBNull.Value) { Obj.InvoiceCompanyName = string.Empty; } else { Obj.InvoiceCompanyName = (string)sdr["InvoiceCompanyName"]; }
                ...
                try
                {
                    ...
                    foreach (M_Invoice m in Obj)
                    {
                        ...
                        while (sdr.Read())
                        {
                            ...
                            if (sdr["InvoiceCompanyName"] == DBNull.Value) { m.InvoiceCompanyName = string.Empty; } else { m.InvoiceCompanyName = (string)sdr["InvoiceCompanyName"]; }
